...
def perform_bgrl_training(data, output_dir, representation_size, device, input_size: int, has_features: bool, g_zoo, **kwargs):
    ...
    for epoch in tqdm(range(1, FLAGS.epochs + 1), desc="BGRL Training"):
        ...
        # wandb.log({'bgrl_train_loss': loss.item(), 'epoch': epoch})  # Comentado para logs más limpios
        
        # Log progress cada 10 épocas
        if epoch % 10 == 0:
            print(f"[BGRL Encoder] Época {epoch}/{FLAGS.epochs} - Loss: {loss.item():.6f} - LR: {lr:.6f}")
